...
def get():
    ...
    if name != None:
        if flag is 0:
            ...
            url_path+="&name="+name
    if initiator_os_type != None:
        if flag is 0:
            url_path+="?initiator_os_type="+initiator_os_type
            flag=1
        else:
            url_path+="&initiator_os_type="+initiator_os_type
    if initiators != None:
...
def post():
    ...
    if (name != None) & (name != key):
        payload['name']=name
    if (initiator_os_type != None) & (initiator_os_type != key):
...
def main():
        fields = {
                ...
                "initiator_os_type" : {"required": False, "type": "str"},
        ...
        global initiator_os_type
        initiator_os_type = module.params["initiator_os_type"]
        global initiators
