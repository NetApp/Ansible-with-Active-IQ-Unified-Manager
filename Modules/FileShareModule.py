...
def get():
    ...
    if key != None:
        if flag is 0:
            ...
            url_path+="&key="+key
    if primary_file_share_key != None:
        if flag is 0:
            url_path+="?primary_file_share_key="+primary_file_share_key
            flag=1
        else:
            url_path+="&primary_file_share_key="+primary_file_share_key
    if name != None:
    ...
    if is_replica != None:
        if flag is 0:
            url_path+="?is_replica="+is_replica
            flag=1
        else:
            url_path+="&is_replica="+is_replica
    if is_clone != None:
        if flag is 0:
            ...
            url_path+="&is_clone="+is_clone
    if protection_node_name != None:
        if flag is 0:
            url_path+="?protection_node_name="+protection_node_name
            flag=1
        else:
            url_path+="&protection_node_name="+protection_node_name
    ...
    if last_modified_timestamp != None:
        if flag is 0:
            ...
            url_path+="&last_modified_timestamp="+last_modified_timestamp
    if parent_file_share_key != None:
        if flag is 0:
            url_path+="?parent_file_share_key="+parent_file_share_key
            flag=1
        else:
            url_path+="&parent_file_share_key="+parent_file_share_key
    if sortBy != None:
...
def post():
    ...
    if (key != None) & (key != key):
        payload['key']=key
    if (primary_file_share_key != None) & (primary_file_share_key != key):
        payload['primary_file_share_key']=primary_file_share_key
    ...
    if (is_replica != None) & (is_replica != key):
        payload['is_replica']=is_replica
    ...
    if (parent_file_share_key != None) & (parent_file_share_key != key):
        payload['parent_file_share_key']=parent_file_share_key
...
def main():
        fields = {
                ...
                "primary_file_share_key" : {"required": False, "type": "str"},
                ...
                "is_replica" : {"required": False, "type": "str"},
                ...
                "parent_file_share_key" : {"required": False, "type": "str"},
        ...
        global primary_file_share_key
        primary_file_share_key = module.params["primary_file_share_key"]
        global name
        ...
        global is_replica
        is_replica = module.params["is_replica"]
        global is_clone
        ...
        global parent_file_share_key
        parent_file_share_key = module.params["parent_file_share_key"]
        global sortBy
