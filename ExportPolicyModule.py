...
def get():
    ...
    if ro_hosts != None:
        if flag is 0:
            url_path+="?ro_hosts="+ro_hosts
            flag=1
        else:
            url_path+="&ro_hosts="+ro_hosts
    if rw_hosts != None:
        if flag is 0:
            url_path+="?rw_hosts="+rw_hosts
            flag=1
        else:
            url_path+="&rw_hosts="+rw_hosts
    if root != None:
    ...
    if security_type != None:
        if flag is 0:
            url_path+="?security_type="+security_type
            flag=1
        else:
            url_path+="&security_type="+security_type
...
def post():
    ...
    if (ro_hosts != None) & (ro_hosts != key):
        payload['ro_hosts']=ro_hosts
    if (rw_hosts != None) & (rw_hosts != key):
        payload['rw_hosts']=rw_hosts
    if (root != None) & (root != key):
        payload['root']=root
    if (security_type != None) & (security_type != key):
...
def main():
        fields = {
                ...
                "ro_hosts" : {"required": False, "type": "str"},
                "rw_hosts" : {"required": False, "type": "str"},
                "root" : {"required": False, "type": "str"},
                "security_type" : {"required": False, "type": "str"},
        ...
        global ro_hosts
        ro_hosts = module.params["ro_hosts"]
        global rw_hosts
        rw_hosts = module.params["rw_hosts"]
        global root
